...
export default function AppointmentForm() {
  ...
  const stockId = searchParams.get("stock"); // Changed from varianId to stockId
  const varianId = searchParams.get("varian"); // Keep for backward compatibility
  ...
  // Handle auto-filling from URL parameters
  useEffect(() => {
    if (stockId && (stokMobils as any) && Array.isArray(stokMobils as any)) {
      // Direct stock selection from catalog
      const selectedStock = (stokMobils as any[]).find(
        (stok: any) => stok.id.toString() === stockId
      );
      if (selectedStock) {
        setValue("stok_mobil_id", selectedStock.id);
      }
    } else if (
      varianId &&
      (stokMobils as any) &&
      ...
      // Backward compatibility: find stock by varian (legacy)
      const selectedVariant = (stokMobils as any[]).find(
        (stok: any) => stok.varian_id?.toString() === varianId
      );
      if (selectedVariant) {
        setValue("stok_mobil_id", selectedVariant.id);
      }
    }
  }, [stockId, varianId, stokMobils, setValue]);
      ...
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Terima kasih telah membuat janji temu dengan kami. Tim kami akan
            menghubungi Anda dalam waktu 1x24 jam untuk konfirmasi jadwal.
          </p>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="nama_pelanggan">Nama Lengkap *</Label>
                    <Input
                      id="nama_pelanggan"
                  ...
                  <div>
                    <Label htmlFor="email_pelanggan">Email *</Label>
                    <Input
                      id="email_pelanggan"
                      type="email"
                  ...
                  <div>
                    <Label htmlFor="telepon_pelanggan">Nomor WhatsApp *</Label>
                    <Input
                    ...
                    {errors.telepon_pelanggan && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.telepon_pelanggan.message}
                      </p>
                    )}
                  </div>
              ...
              <div>
                ...
                {mobilId && mobil ? (
                  <div className="bg-gray-50 p-4 rounded-lg mb-4">
                    <h4 className="font-medium">{(mobil as any).nama_mobil}</h4>
                  ...
                  <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-4">
                    <div className="flex items-center gap-2 text-blue-800">
                      ...
                      <span className="text-sm">
                        Anda dapat memilih mobil spesifik atau melakukan
                        konsultasi umum
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  <Label htmlFor="stok_mobil_id">Pilih Mobil (Opsional)</Label>
                  <Select
                    value={watch("stok_mobil_id")?.toString() || ""}
                    onValueChange={(value) => {
                      if (value === "none") {
                        setValue("stok_mobil_id", undefined as any);
                      } else {
                        setValue("stok_mobil_id", Number(value));
                      }
                    }}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Pilih mobil atau kosongkan untuk konsultasi umum" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">
                        Tidak ada (Konsultasi umum)
                      </SelectItem>
                      {(stokMobils as any) &&
                        Array.isArray(stokMobils as any) &&
                        (stokMobils as any[]).map((stok: any) => (
                          <SelectItem key={stok.id} value={stok.id.toString()}>
                            {stok.nama_lengkap ||
                              `${stok.varian} - ${stok.warna}`}{" "}
                            - {formatCurrency(stok.harga_jual || stok.harga)}
                          </SelectItem>
                        ))}
                    </SelectContent>
                  </Select>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <Label htmlFor="metode">Metode Pertemuan *</Label>
                    <Select
                ...
                {selectedMetode === "offline" && (
                  <div className="mb-4">
                    <Label htmlFor="lokasi">Lokasi Pertemuan *</Label>
                    <Select
                      value={watch("lokasi")}
                      onValueChange={(value) =>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="showroom">Showroom Kami</SelectItem>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <Label htmlFor="waktu_mulai">Tanggal & Waktu Mulai *</Label>
                    <Input
                      id="waktu_mulai"
                      type="datetime-local"
                ...
                <div className="mb-4">
                  <Label htmlFor="waktu_alternatif">Waktu Alternatif</Label>
                  <Input
                    id="waktu_alternatif"
                    type="datetime-local"
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Opsional: Waktu alternatif jika jadwal utama tidak tersedia
                  </p>
                </div>

                <div className="mb-4">
                  <Label htmlFor="tujuan">Tujuan Khusus</Label>
                  <Input
                    id="tujuan"
                    {...register("tujuan")}
      ...
      <div className="space-y-6">
        ...
        {selectedVariant ? (
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Mobil Dipilih</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="font-medium">
                    {selectedVariant.nama_lengkap || selectedVariant.varian}
                  </p>
                  <p className="text-xl font-bold text-primary">
                    {formatCurrency(
                      selectedVariant.harga_jual || selectedVariant.harga
                    )}
                  </p>
                  <div className="flex gap-2 mt-2">
                    <Badge variant="secondary">{selectedVariant.warna}</Badge>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Jenis Janji Temu</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-center py-4">
                ...
                <p className="text-sm text-gray-600">
                  Anda akan melakukan konsultasi umum tanpa mobil spesifik. Tim
                  kami akan membantu menemukan mobil yang sesuai kebutuhan.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium">info@tokojayamotor.com</p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2 text-sm text-gray-600">
              ...
              <p>
                • Konsultasi umum juga tersedia tanpa memilih mobil spesifik
              </p>
            </div>
          </CardContent>
        </Card>

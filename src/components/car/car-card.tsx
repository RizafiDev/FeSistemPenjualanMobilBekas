...
export default function CarCard({ car, stockItem, className }: CarCardProps) {
  const primaryPhoto =
    car.foto_mobils?.find((foto) => foto.is_primary) || car.foto_mobils?.[0];
  const carSlug = getCarSlug(car);
  const lowestPrice = stockItem?.harga_jual || car.stok_mobils?.[0]?.harga_jual;
    ...
    >
      <div className="relative overflow-hidden rounded-t-lg">
        ...
        <div className="absolute top-3 left-3">
          <Badge
            variant={stockItem?.status === "tersedia" ? "default" : "secondary"}
          >
        ...
        {stockItem?.kondisi && (
          <div className="absolute top-3 right-3">
            <Badge
              variant={stockItem.kondisi === "baru" ? "default" : "outline"}
            >
      ...
      <CardContent className="p-4">
        <div className="space-y-2">
          ...
          {/* Car Name */}
          <h3 className="font-semibold text-lg line-clamp-1">{car.nama}</h3>
          {/* Year and Price */}
          <div className="flex items-center justify-between">
            {" "}
          ...
          {stockItem && (
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              {stockItem.kilometer && (
                <span>{stockItem.kilometer.toLocaleString()} km</span>
              )}
            </div>
          )}
          {/* Description */}
